...
class MycelialNetwork:
    ...
    def update(self):
        ...
        if n_tips > 0:
            ...
            # Alternative Method 2: Velocity-based selection (uncomment to use)
            # if hasattr(self, 'prev_positions') and n_pullers > 0:
            #     velocities = np.linalg.norm(self.tip_positions - self.prev_positions, axis=1)
            #     puller_indices = np.argpartition(velocities, -n_pullers)[-n_pullers:]
            self.prev_positions = self.tip_positions.copy()
